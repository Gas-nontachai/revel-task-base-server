...
const invoiceSupplierDetailHtml = ({ invoice_supplier, invoice_supplier_lists, }) => {
...
const addInvoice = async (connection, data = []) => {
  ...
  const icon = fs.readFileSync(__dirname + '/assets/images/icon-info.png', 'binary')
  ...
  return {
    mails: data.map(item => {
      ...
      const item_url = `<a href="${process.env.TT_CLIENT_URL}/invoice-supplier/detail?id=${invoice_supplier_id}">${invoice_supplier_origin_id}</a>`

      return {
        subject: `(TTCenter) มีรายการใหม่ ${invoice_supplier_origin_id} - ${invoice_supplier_branch_name}`,
        html: `
          <html>
            <head>
              <style>${style}</style>
            ...
            <body>
              <div class="container">
                ...
                <p>เมื่อ ${formatDate(invoice_supplier_key_date, 'HH:mm dd/MM/yyyy')}</p>
...
const updateInvoice = async (connection, data = []) => {
  ...
  const icon = fs.readFileSync(__dirname + '/assets/images/icon-warning.png', 'binary')
  ...
  return {
    mails: data.map(item => {
      ...
      return {
        subject: `(TTCenter) มีการอัปเดตใบ ${invoice_supplier_origin_id} - ${invoice_supplier_branch_name}`,
        html: `
          <html>
            <head>
              <style>${style}</style>
