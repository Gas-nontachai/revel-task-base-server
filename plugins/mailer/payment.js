...
const invoiceSupplierDetailHtml = (data) => {
  ...
  const list_html = invoice_supplier_lists.map((item, idx) => (`
    <tr>
      ...
      <td class="text-right">${decimalFix(item.invoice_supplier_list_net_weight)}</td>
      <td class="text-right">${decimalFix(item.invoice_supplier_list_unit_price)}</td>
      <td class="text-right">${decimalFix(item.invoice_supplier_list_price)}</td>
    </tr>
  `)).join('')
  ...
  return `
    ...
    <table class="w-100">
      ...
      <tfoot>
        <tr>
          <td colspan="3" rowspan="2">หมายเหตุ : ${invoice_supplier.invoice_supplier_remark ?? '-'}</td>
          <td class="text-right">น้ำหนักรวม</td>
          <td class="text-right">${decimalFix(invoice_supplier.invoice_supplier_weight)} กิโลกรัม</td></tr>
        </tr>
        <tr>
          <td class="text-right">ยอดรวม</td>
          <td class="text-right">${decimalFix(invoice_supplier.invoice_supplier_net_price)} บาท</td>
        </tr>
      </tfoot>
    </table>
  `
...
const paymentPaid = async (connection, data) => {
  ...
  const supplier = await SupplierModel.getSupplierByID(connection, { supplier_id: invoice_supplier.supplier_id })
  const user = await UserModel.getUserByID(connection, { user_id })

  const { docs: invoice_supplier_lists } = await InvoiceSupplierListModel.getInvoiceSupplierListBy(connection, { match: { invoice_supplier_id }, })
  ...
  const icon = fs.readFileSync(__dirname + '/assets/images/icon-success.png', 'binary')
  const item_url = `<a href="${process.env.TT_CLIENT_URL}/invoice-supplier/detail?id=${invoice_supplier_id}">${invoice_supplier.invoice_supplier_origin_id}</a>`

  return {
    mails: [{
      subject: `(TTCenter) ${invoice_supplier.invoice_supplier_origin_id} - ${invoice_supplier.invoice_supplier_branch_name} มีการชำระแล้ว`,
      html: `
        <html>
          <head>
            <style>${style}</style>
          ...
          <body>
            <div class="container">
              ...
              <p>ได้รับการชำระแล้วเมื่อ ${formatDate(new Date(), 'HH:mm dd/MM/yyyy')}</p>
              <p>โดย ${user.user_fullname}</p>
              ${invoiceSupplierDetailHtml({ invoice_supplier, supplier, invoice_supplier_lists })}
            </div>
          </body>
        </html>
      `,
      attachments: [{
        path: `data:image/jpg;base64,${Buffer.from(icon, 'binary').toString('base64')}`,
        cid: 'subject-icon'
      }],
    }],
...
const paymentInvalid = async (connection, data) => {
  ...
  const icon = fs.readFileSync(__dirname + '/assets/images/icon-danger.png', 'binary')
  const item_url = `<a href="${process.env.TT_CLIENT_URL}/invoice-supplier/detail?id=${invoice_supplier_id}">${invoice_supplier.invoice_supplier_origin_id}</a>`

  return {
    mails: [{
      subject: `(TTCenter) ${invoice_supplier.invoice_supplier_origin_id} - ${invoice_supplier.invoice_supplier_branch_name} ยอดชำระไม่ถูกต้อง`,
      html: `
        <html>
          <head>
            <style>${style}</style>
